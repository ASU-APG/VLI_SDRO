...
@torch.no_grad()
def evaluate(model, eval_loader, label2ans, save_logits=False, ans2label=None):
    ...
    for i, batch in enumerate(eval_loader):
        ...
        # print(scores)
        m = scores.max(dim=-1, keepdim=False)[1].cpu().tolist()
       
        prob_scores_all = torch.softmax(scores, dim=-1).tolist()
        prob_scores_yes_no = []
        for j, item in enumerate(prob_scores_all):
            prob_scores_yes_no.append({"yes": item[yes_idx], "no": item[no_idx]})
            assert m[j] == yes_idx or m[j] == no_idx


        # print(prob_scores)
        # print(scores.size(), m.size(), len(prob_scores), len(prob_scores[0]))
        # print(prob_scores[0])
        # break
        targets = batch['targets']
        m_target = targets.max(dim=-1, keepdim=False)[1].cpu().tolist()
        # print(m_target)
        # break
        ...
        answers = [label2ans[i] for i in m]
        gt_answers = [label2ans[i] for i in m_target]

        for qid, answer, prob, gt_answer in zip(qids, answers, prob_scores_yes_no, gt_answers):
            if answer == gt_answer:
                count1 += 1
            else: 
                count2 += 1
            results.append({'pred_answer': answer, 'gt_answer': gt_answer, 'question_id': int(qid), 'confidence' : prob})
