...
from data import (VqaDataset_DRO, vqa_collate_dro, 
                    VqaAugDataset_DRO, vqa_collate_aug_dro)
...
def main(opts):
    ...
    while True:
        ...
        if n_epoch >= opts.n_pre and n_epoch < opts.n_post:
            ...
            print(train_dataset)
            for i, _t in enumerate(train_datasets):
                print(i, _t)
                eval_aug_dset, x = _t.get_aug_data(n_pre=opts.n_pre, 
                                                    n_post=opts.n_post, 
                                                    T = opts.T)
                eval_aug_dataloader = build_dataloader(eval_aug_dset, vqa_collate_aug_dro, True, opts)
                aug_li = []
                aug_li = perform_eval_on_aug(model, eval_aug_dataloader, x, opts)
                # print("Len of aug_li ", len(aug_li))
                _t.add_aug_data(aug_li)        
...
def perform_eval_on_aug(model, aug_eval_loader, x, opts):
    ...
    final_aug_data_li = []
    save_li = []
    # eval_loss = F.binary_cross_entropy_with_logits(reduction='none')
    # eval_loss = nn.CrossEntropyLoss(ignore_index=-1, reduce = False)
    for i, batch in enumerate(aug_eval_loader):
        # qids = batch['qids']
        targets = batch['targets']
        parent_ids, tags, db_ids =  batch['parents'], batch['tags'], batch['question_ids']
        del batch['targets']
        # del batch['qids']
    ...
    # print(loss_li[0], parent_li[0], tag_li[0], data_id_li[0])
