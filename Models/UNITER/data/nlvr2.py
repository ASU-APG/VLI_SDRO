...
class Nlvr2PairedDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db, use_img_type=True):
        ...
        self.txt_db = txt_db
        self.img_db = img_db
        _txt_lens, _ids = get_ids_and_lens(txt_db)
        print("Len of ids ", len(_ids))
        txt2img = txt_db.txt2img
        
        # Remove the ids which don't have corresponding images
        txt_lens, self.ids = [], []
        for tl, id_ in zip(_txt_lens, _ids):
            flag = True
    ...
    def __getitem__(self, i):
        ...
        # if 'orig_label' in example and example['orig_label'] == 0:
        #         print(example['uid'])
        outs = []
