...
def get_ids_and_lens(db):
    ...
    for id_ in list(db.id2len.keys())[hvd.rank()::hvd.size()]:
        item  = db.__getitem__(id_)
        # Getting only positive statements and original
        # if item['tag'] == 'orig' or item['orig_label'] == 1:
...
def get_ids_and_lens_stat(db):
    ...
    aug_ids = []
    aug_lens = []
    orig_transformation_dict = {}
    for id_ in list(db.id2len.keys())[hvd.rank()::hvd.size()]:
        item  = db.__getitem__(id_)
        if item['tag'] == 'orig':
            orig_lens.append(db.id2len[id_])
            orig_ids.append(id_)
        # elif item['orig_label'] == 1:
        else:  
            aug_lens.append(db.id2len[id_])
            aug_ids.append(id_)
            parent_id = item['parent_identifier']
            if parent_id not in orig_transformation_dict:
                orig_transformation_dict[parent_id] = []
            orig_transformation_dict[parent_id].append(id_)

    return orig_lens, orig_ids,  orig_transformation_dict

def get_ids_and_lens_stat_vqa(db):
    # assert isinstance(db, TxtTokLmdb)
    lens = []
    ids = []
    orig_ids = []
    ...
    for id_ in list(db.id2len.keys())[hvd.rank()::hvd.size()]:
        ...
        # elif item['orig_label'] == 1:
            ...
            aug_lens.append(db.id2len[id_])
            aug_ids.append(id_)
            parent_id = str(item['parent_question_id'])
            if parent_id not in orig_transformation_dict:
                orig_transformation_dict[parent_id] = []
            orig_transformation_dict[parent_id].append(id_)
