...
def create_dataloader_sisp(img_path, txt_path, txt_sisp_path, batch_size, is_train,
                      dset_cls, collate_fn, opts):
    img_db = DetectFeatLmdb(img_path, opts.conf_th, opts.max_bb, opts.min_bb,
                            opts.num_bb, opts.compressed_db)
    txt_db = TxtTokLmdb(txt_path, opts.max_txt_len if is_train else -1)
    dset_orig = dset_cls(txt_db, img_db, opts.use_img_type)

    txt_sisp_db = TxtTokLmdb(txt_sisp_path, opts.max_txt_len if is_train else -1)

    dset_sisp = dset_cls(txt_sisp_db, img_db, opts.use_img_type)
...
def main(opts):
    ...
    # data loaders
    train_orig_dataloader = create_dataloader(opts.train_img_db, opts.train_txt_db,
    ...
    test_dataloader = create_dataloader(opts.test_img_db, opts.test_txt_db,
                                        opts.val_batch_size, False,
                                        EvalDatasetCls, eval_collate_fn, opts)
    test_sisp_dataloader = create_dataloader(
                                opts.test_img_db, opts.test_sisp_txt_db, 
                                opts.val_batch_size, True, EvalDatasetCls, 
                                collate_fn, opts)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_sisp_txt_db",
                        default=None, type=str,
                        help="The input train tranformed corpus. (LMDB)")
    ...
    parser.add_argument("--test_sisp_txt_db",
                        default=None, type=str,
                        help="The input test corpus. (LMDB)")
